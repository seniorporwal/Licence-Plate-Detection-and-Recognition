...
def save_output_to_txt(output_string, file_path):
...
class DetectionPredictor(BasePredictor):
    ...
    def write_results(self, idx, preds, batch):
        p, im, im0 = batch
        output_dir = str(self.save_dir)  # Get the directory where other output files are stored
        ...
        for *xyxy, conf, cls in reversed(det):
            ...
            if self.args.save or self.args.save_crop or self.args.show:  # Add bbox to image
                ...
                if ocr != "":
                    label = ocr
                    #attach a output text file with the results video with confidence per frame
